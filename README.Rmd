...
Note that all text rendering is performed through a custom-built rendering pipeline that is part of the gridtext package. This approach has several advantages, including minimal dependencies, good peformance, and compatibility with all R graphics devices (to the extent that the graphics devices support the fonts you want to use). The downside of this approach is the severely limited featureset. Don't expect this package to support the fancy CSS and javascript tricks you're used to when designing web pages.
...
The alignment parameters `hjust`, `vjust`, `box_hjust`, and `box_vjust` function just like they do in `richtext_grob()`.
