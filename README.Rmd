...
The function `rich_text_grob()` can be used to draw simple html-like text. It is currently not vectorized, but the ultimate goal is to write a variant of `labels_grob()` that can make use of it. To get placement right, this currently needs to be enclosed in a `box_grob()`. 
...
grid.draw(
  box_grob(
    rich_text_grob(text1), x = 0., y = 1, hjust = 0, vjust = 1
  )
)
grid.draw(
  box_grob(
    rich_text_grob(text2, hjust = 0),
    x = 0.3, y = 0.5, angle = 45
  )
)
grid.draw(
  box_grob(
...
grid.draw(box_grob(rich_text_grob(text3), x = 0.2, y = 0.1, padding = mar(5, 5, 5, 5), debug = TRUE))
```
